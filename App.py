...
def carregar_dados_mestre():
    ...
    df['Target'] = (df['Algodao'].shift(-1) > (df['Algodao'] * 1.001)).astype(int)
...
def get_market_status():
    ...
    return ("🟢 MERCADO ABERTO", "Fecha às 17h", "#104a10") if abertura <= agora <= fechamento else ("🔴 MERCADO FECHADO", "Abre em breve", "#4a1010")
...
st.markdown("""
    ...
    .news-card { background-color: #161b22; padding: 12px; border-radius: 8px; margin-bottom: 8px; border-left: 4px solid #58a6ff; font-size: 0.9rem; }
    </style>
...
try:
    ...
    with st.sidebar:
        ...
        risco = st.slider("Risco/Trade %", 0.5, 5.0, 1.5)
        
        if not historico.empty:
            win_rate = (len(historico[historico['lucro'] > 0]) / len(historico)) * 100
            st.metric("Win Rate IA", f"{win_rate:.1f}%")
    ...
    col_ia, col_op = st.columns([1.5, 1])

    with col_ia:
        cor_ia, txt_ia = ("#deff9a", "COMPRA FORTE") if prob > 0.75 else ("#ff4b4b", "VENDA FORTE") if prob < 0.25 else ("#fccf03", "AGUARDAR")
        st.markdown(f'<div class="ia-container" style="border-color: {cor_ia}; color: {cor_ia};"><small style="color:#8b949e">PROBABILIDADE DE SUCESSO</small><br><span style="font-size: 55px; font-weight: 900;">{prob*100:.1f}%</span><br><b style="font-size: 22px;">{txt_ia}</b></div>', unsafe_allow_html=True)
    ...
    with col_op:
        ...
        if 'ent' not in st.session_state:
            ...
            if st.button("✅ FECHAR E COMPUTAR", use_container_width=True):
                ...
                c.execute('INSERT INTO trades (data, tipo, entrada, saida, lucro, confianca) VALUES (?,?,?,?,?,?)',
                         (datetime.now().strftime("%H:%M:%S"), st.session_state.tipo, st.session_state.ent, preco_atual, lucro, prob))
                c.commit(); c.close()
                del st.session_state.ent
                st.rerun()
        st.markdown('</div>', unsafe_allow_html=True)

    t1, t2, t3 = st.tabs(["📈 Gráfico Real-Time", "📜 Histórico de Trades", "📰 Inteligência de Notícias"])

    with t1:
        fig = go.Figure(data=[go.Candlestick(x=df.index[-50:], open=df['Algodao'][-50:]*0.999, high=df['Algodao'][-50:]*1.001, low=df['Algodao'][-50:]*0.998, close=df['Algodao'][-50:])])
        fig.update_layout(template="plotly_dark", height=400, margin=dict(l=0,r=0,t=0,b=0), xaxis_rangeslider_visible=False)
        st.plotly_chart(fig, use_container_width=True)

    with t2:
        if not historico.empty:
            st.dataframe(historico[['data', 'tipo', 'lucro', 'confianca']].head(10), use_container_width=True)
        else: st.info("Nenhuma operação registrada ainda.")
    ...
    st.error(f"Erro de Conexão: {e}. Aguardando reconexão...")
