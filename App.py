...
try:
    ...
    if not dados_vapt.empty:
        ...
        l_v = (-d_v.where(d_v < 0, 0)).rolling(window=5).mean()
        rs_v = g_v / (l_v + 1e-9)
        ...
        # Algoritmo de Pontuação de Reação Rápida
        score_sc = 50.0
        if ultimo_preco > ultima_ema: score_sc += 25.0
        else: score_sc -= 25.0
        
        if ultimo_rsi5 > 50: score_sc += 20.0
        else: score_sc -= 20.0
...
# --- DESIGN COM 3 COLUNAS DE SINAL NO TOPO ---
col_dia_box, col_h1_box, col_scalper_box = st.columns(3)
...
with col_dia_box:
    ...
    st.markdown(f"""
        <div class="ia-container" style="border-color: {cor_dia}; color: {cor_dia};">
            ...
            <span style="font-size: 26px; font-weight: 900;">{prob_diaria*100:.1f}%</span><br><b>{txt_dia}</b>
        </div>
        """, unsafe_allow_html=True)
...
with col_h1_box:
    ...
    st.markdown(f"""
        <div class="ia-container" style="border-color: {cor_h1}; color: {cor_h1};">
            ...
            <span style="font-size: 26px; font-weight: 900;">{prob_1h*100:.1f}%</span><br><b>{txt_h1}</b>
        </div>
        """, unsafe_allow_html=True)

with col_scalper_box:
    if prob_scalper >= 55.0:
        cor_sc, txt_sc = "#00CF85", "💥 IGNIÇÃO DE ALTA (SCALP)"
    elif prob_scalper <= 45.0:
        cor_sc, txt_sc = "#ff4b4b", "💥 IGNIÇÃO DE QUEDA (SCALP)"
    else:
        cor_sc, txt_sc = "#fccf03", "MICRO LATERALIDADE"

    st.markdown(f"""
        <div class="ia-container" style="border-color: {cor_sc}; color: {cor_sc}; background-color: rgba(255,255,255,0.02);">
            <small style="color: #58a6ff; font-weight:bold;">⚡ SCALPING AGRESSIVO (1M)</small><br>
            <span style="font-size: 26px; font-weight: 900;">{prob_scalper:.1f}%</span><br><b>{txt_sc}</b>
        </div>
        """, unsafe_allow_html=True)
...
with tab_g:
    ...
    if not dados_vapt.empty:
        try:
            ...
            st.caption("Aguardando transmissão estável de velas...")
    else:
        st.caption("Sincronizando dados de alta velocidade...")
